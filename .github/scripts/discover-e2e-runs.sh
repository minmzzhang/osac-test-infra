...
#   status.env   SKIPPED_TARGETS=N (targets whose run-listing call failed),
...
echo "Listing runs completed since ${SINCE}Z..."
...
for TARGET in "${TARGETS[@]}"; do
  ...
  HTTP_CODE=$(curl -sL -o "${RESP_FILE}" -w '%{http_code}' \
    -H "Authorization: Bearer ${GH_TOKEN}" \
    -H "Accept: application/vnd.github+json" \
    "${GITHUB_API_URL}/repos/${REPO}/actions/workflows/${WORKFLOW}/runs?status=completed&created=%3E${SINCE}&per_page=100")
  if [[ "${HTTP_CODE}" != "200" ]]; then
    echo "::warning::Could not list runs for ${TARGET} (HTTP ${HTTP_CODE}), skipping."
    SKIPPED_TARGETS=$((SKIPPED_TARGETS + 1))
    continue
  fi
  IDS=$(jq --arg repo "${REPO}" '[.workflow_runs[]? | {run_id: (.id | tostring), repo: $repo}]' "${RESP_FILE}")
...
echo "SKIPPED_TARGETS=${SKIPPED_TARGETS}" > "${OUTPUT_DIR}/status.env"
