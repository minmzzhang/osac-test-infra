...
- `.github/workflows/e2e-on-label.yml` — Canonical starter (`lgtm` / `e2e-ready` only; `/ok-to-test` does not start e2e or POST gates). osac calls it via `workflow_call`. `/e2e-ready` `workflow_dispatch`es the per-repo wrapper. Replay bash: `.github/actions/e2e-start/` (do not POST in_progress `e2e-*-gate` checks when every caller would skip rerun, or when the starter run is not on the PR SHA — `/e2e-ready` dispatch runs on main and unbound posts land on unrelated suites such as ok-to-test cleanup)
