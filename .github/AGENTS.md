...
- `.github/workflows/e2e-on-label.yml` — Canonical starter (`lgtm` / `e2e-ready`). osac calls it via `workflow_call`. `/e2e-ready` `workflow_dispatch`es the per-repo wrapper. Replay bash: `.github/actions/e2e-start/` (do not POST in_progress `e2e-*-gate` checks when every caller would skip rerun — e.g. `lgtm` while CodeRabbit full-install is already running)
